...
describe(useLocalStorage, () => {
  ...
  it('should retrieve actual value when there is something in store', async () => {
    ...
    await waitFor( () => { expect(result.current[0]).not.toBe(LOADING) }); 
    const [state] = result.current; 
  ...
  it("should prperly return new value ",async () => {
    ...
    await waitFor( () => { expect(result.current[0]).not.toBe(LOADING) }); 
    const [, setFoo] = result.current;
    await act(async () => await setFoo('baz'));
  ...
  it('should keep in sync two hooks based on the same key', async () => {
    ...
    await waitFor( () => { expect(r1.current[0]).not.toBe(LOADING) }); 
    await waitFor( () => { expect(r2.current[0]).not.toBe(LOADING) }); 
  ...
  it('should work with function updater', async () => {
    ...
    await waitFor( () => { expect(result.current[0]).not.toBe(LOADING) }); 
    const [, setFoo] = result.current;
    await act(async () => await setFoo((state) => `${state?.toString()}_bar`));

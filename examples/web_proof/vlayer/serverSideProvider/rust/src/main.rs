...
async fn main() {
    ...
    let args = Args::parse();
    let parsed_url = Url::parse(&args.url).expect("Invalid URL");

    // Extract server domain, path, and port from the URL
    let server_domain = parsed_url.host_str().unwrap_or("example.com");
    let path = parsed_url.path();
    let port = parsed_url.port_or_known_default().unwrap_or(443); // default to 443
    ...
    let client_socket = tokio::net::TcpStream::connect((server_domain, port))
        .await
        .unwrap();

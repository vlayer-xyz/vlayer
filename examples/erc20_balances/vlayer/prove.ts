...
let token: Address = await testHelpers.deployContract(vToyken);
let prover: Address = await testHelpers.deployContract(erc20Prover, [token]);
console.log(`Prover has been deployed on ${prover} address`);


console.log("Proving...");
let response = await prove(prover, erc20Prover, 'prove', ARGS);

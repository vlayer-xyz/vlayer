...
### Outgoing server 
The vlayer prover verifies that the message signature matches the public key listed in the DNS records. However, a dishonest outgoing server can forge emails and deceive the prover into generating valid proofs for them. Therefore, vlayers support only a limited number of the world's most trusted email providers. 

### Prevent unauthorized actions
Both outgoing and incoming servers can read emails and use them to create proofs without the permission of the actual mail sender or receiver. This risk also extends to the prover, which access the email to generate claim. It is important for protocols to utilize email proofs in a manner that prevents the manipulation of smart contracts into performing unauthorized actions, such as sending funds to unintended recipients.

For example, it is advisable to include complete information in the email to ensure correct actions. Prefer emails like: "Send 1 ETH from address X to address Y on Ethereum Mainnet" over partical instructions like: "Send 1 ETH.", where other details comes from different source (e.g. smart contract call parameters). Other approach is to use unique identifiers, which point unambiguously to necceary details. 
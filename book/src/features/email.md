...
* `MultiSigWallet` inherits from `Verifier`, so we can call the `onlyVerified` modifier that makes sure the proof is correct (it will revert otherwise).
...
* The `recovery()` function takes Prover and public inputs (generated by `Prover.main()`) as arguments.
* `onlyVerified(PROVER_ADDR, PROVER_FUNC_SELECTOR)` validates proof, false proof or public input stops execution and prevents wallet recovery 
* `ownerToEmailHash[lostWallet] == emailHash` make sure recovery email hasn't changed over time, otherwise revert

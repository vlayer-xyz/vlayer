...
The `Email Notary` (aka. Verifiable DNS) service exists for this reason: it uses the [DNS Queries over HTTPS (DoH)](https://datatracker.ietf.org/doc/html/rfc8484) protocol to fetch DNS records from several providers, signs them if they are valid and secure, and returns the signature together with the record.

...
TMP_DIR=$( mktemp -d -t vlayer-$(basename $0).XXXXX )
...
wait_for_port() {
    ...
    timeout --foreground --kill-after=5s "${timeout}" bash -c "until nc -z localhost ${port}; do sleep 3; done"
...
function startup_vlayer(){
    ...
    (
        ...
        2>"${LOGS_DIR}/vlayer_serve.err" & 

        VLAYER_SERVER_PID=$!

        echo "vlayer server started with PID ${VLAYER_SERVER_PID}."
        wait_for_port 3000 30m "vlayer server"
...
function deploy_contracts() {
    ...
    (    
        ...
        source ../../bash/vlayer-deploy.sh ${example} | \
             tee ${LOGS_DIR}/deployment.out | \
             grep '[A-Z0-9_]\+=[a-zA-Z0-9]\+' >"${TMP_DIR}/deployment.env" 
    ) 
    ...
    set -a ; source "${TMP_DIR}/deployment.env" ; set +a
    eval "EXAMPLE_ADDRESS_VAR=\${${EXAMPLE}}"
    eval "echo \${${EXAMPLE_ADDRESS_VAR}}"
}
...
EXAMPLE_PROVER_ADDRESS=$(deploy_contracts "${EXAMPLE}")

# echo "Sending v_call request to vlayer JSON-RPC server"
...
# curl -sX POST http://127.0.0.1:3000/ -H "Content-Type: application/json" -d "$v_call_request_json"

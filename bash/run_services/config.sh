...
function set_external_rpc_urls() {
    if [[ "${CHAIN_NAME}" != "anvil" ]]; then
        if [[ -n "${ALCHEMY_API_KEY:-}" ]]; then
        ...
        elif [[ -n "${QUICKNODE_API_KEY:-}" && -n "${QUICKNODE_ENDPOINT:-}" ]]; then
            EXTERNAL_RPC_URLS=(
            ...
            echo "Error: Either ALCHEMY_API_KEY or both QUICKNODE_API_KEY and QUICKNODE_ENDPOINT must be set in prod mode."
            usage
        fi
    fi
}
...
function set_chain_worker_args() {
    ...
    if [[ "${CHAIN_NAME}" == "anvil" ]]; then
        ...
        if [ "${EXAMPLE_NAME:-}" == "simple-time-travel" ]; then
            if [[ -n "${ALCHEMY_API_KEY:-}" ]]; then
                CHAIN_WORKER_ARGS+=(
                    "https://opt-sepolia.g.alchemy.com/v2/${ALCHEMY_API_KEY} 11155420"
                )
            else
                CHAIN_WORKER_ARGS+=(

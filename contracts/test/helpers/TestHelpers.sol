...
contract TestHelpers {
    RiscZeroMockVerifier public immutable mockVerifier =
        new RiscZeroMockVerifier(FAKE_VERIFIER_SELECTOR);

    function createProof(
        ExecutionCommitment memory commitment,
        bytes memory journalParams
        ...
        bytes memory seal = mockVerifier
            .mockProve(ImageID.RISC0_CALL_GUEST_ID, journalHash)
            .seal;
    ...
    function createProof() public view returns (Proof memory, bytes32) {
        ExecutionCommitment memory commitment = ExecutionCommitment(
            PROVER,
            SELECTOR,
            block.number - 1,
    ...
    function setSealProofMode(
        Seal memory seal,
        ProofMode proofMode
    ) public pure returns (Seal memory) {
    ...
    function encodeWordsFake(
        ...
        for (
            uint256 i = SealLib.VERIFIER_SELECTOR_LENGTH - 1;
            i < seal.length;
            ++i
        ) {
            rawSeal <<= 8;

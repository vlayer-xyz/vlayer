...
contract SealLib_decode_Tests is Test {
    ...
    function test_empty() public pure {
        ...
        Seal memory seal = Seal(
            SealFixtures.MOCK_SELECTOR,
            SealFixtures.EMPTY_WORDS(),
            ProofMode.GROTH16
        );
...
contract SealLib_proofMode_Tests is Test {
    ...
    function test_proofMode() public pure {
        Seal memory sealGroth16 = Seal(
            SealFixtures.MOCK_SELECTOR,
            SealFixtures.MAX_WORDS(),
            ProofMode.GROTH16
        );
        Seal memory sealFake = Seal(
            SealFixtures.MOCK_SELECTOR,
            SealFixtures.MAX_WORDS(),
            ProofMode.FAKE
        );
    ...
    function test_proofModeIsEncodedOn288thByte() public pure {
        ...
        assertEq(
            encodedGroth16[PROOF_MODE_ENC_LOCATION],
            bytes1(uint8(ProofMode.GROTH16))
        );
        assertEq(
            encodedFake[PROOF_MODE_ENC_LOCATION],
            bytes1(uint8(ProofMode.FAKE))
        );
    }

...
use eyre::Result;
use foundry_common::{contracts::{ContractsByAddress}, sh_warn, TestFilter, TestFunctionExt, TestFunctionKind};
...
use foundry_evm::{
    ...
    executors::{
        ...
        CallResult, EvmError, ITest, RawCallResult,
    },
    fuzz::{
        fixture_name,
...
use std::sync::Arc;
use forge::{multi_runner::TestContract, result::{SuiteResult, TestResult, TestSetup}, MultiContractRunner};
use forge::multi_runner::TestRunnerConfig;
use foundry_evm::executors::Executor;
use tracing::{debug, debug_span, enabled, error, trace, Span};
use crate::{
    forked::{
...
impl<'a> ContractRunner<'a> {
    ...
    fn _setup(&mut self, call_setup: bool) -> Result<TestSetup> {
        ...
        self.executor.inner.set_balance(self.sender, U256::MAX)?;
        self.executor.inner.set_balance(CALLER, U256::MAX)?;

        // We set the nonce of the deployer accounts to 1 to get the same addresses as DappTools.
        self.executor.inner.set_nonce(self.sender, 1)?;

        // Deploy libraries.
        self.executor.inner.set_balance(LIBRARY_DEPLOYER, U256::MAX)?;
        ...
        // construction
        self.executor.inner.set_balance(address, self.initial_balance())?;

        // Deploy the test contract
        let deploy_result = self.executor.inner.deploy(
            self.sender,
        ...
        self.executor.inner.set_balance(self.sender, self.initial_balance())?;
        self.executor.inner.set_balance(CALLER, self.initial_balance())?;
        self.executor.inner.set_balance(LIBRARY_DEPLOYER, self.initial_balance())?;

        self.executor.inner.deploy_create2_deployer()?;

        // Optionally call the `setUp` function
        if call_setup {
            trace!("calling setUp");
            let res = self.executor.inner.setup(None, address, Some(&self.mcr.revert_decoder));

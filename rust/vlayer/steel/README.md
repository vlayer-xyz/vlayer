...
fn main() {
    ...
    let call_builder = CallBuilder::new(CONTRACT, &CALL).from(CALLER);
    let returns = guest_evm_call(call_builder, &view_call_env);
...
let call_builder = CallBuilder::new(CONTRACT, &CALL).from(CALLER);
let returns = evm_call(call_builder, &mut env);
